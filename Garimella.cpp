...
//			cout<<hminp<<"hminp"<<endl;
	...
	g=(CA*(5.0/16.0+15.0/4.0*pow(cos(CA),2.0) +5.0/2.0*pow(cos(CA),4.0))
	  -sin(CA)*(113.0/48.0*cos(CA) +97.0/24.0*pow(cos(CA),3.0) +1.0/6.0*pow(cos(CA),5.0)));//Ψ(θo))
				...
				A[i]=(log(X[i]+(1-X[i])*Re/Reb)-log(X[i]))/9.0;//exponential coefficient describing wetting ratio increasing for previous tubes
	...
	aave=0;
		for(i=1;i<N;i++){
			...
			if(X[i]==1){
				...
				h[i]=pow(3.0*m*myu/((X[i]+(1-X[i])*Re/Reb)*exp(-A[i]*(j)))/(rho*rho*gl*sin(beta*i)),1.0/3.0);
			    a[i]=8.0/5.0*k/h[i]*(X[i]+(1-X[i])*Re/Reb)*exp(-A[i]*(j));//local HTC calculated with exponential law-increasing wetting ratio
				aave+=a[i]/N;
			}
