 ...
 *  温度		│ ℃     │ sc_tl    │		  │          │ sh_tv    │（圧力[kPa],比エンタルピ[kJ/kg])※陰解法
 *  ――――――┴――――┴―――――┴―――――┴―――――┴―――――┴――
...
出力:換算飽和圧力[-]
**************************/
double PropertyWater::k_func( double _T ){
	...
	x2 = 1.0 + k[6] * (1.0-_T) + k[7] * pow(1.0-_T,2.0);
	x3 = 1.0 - _T;
	x4 = k[8] * pow(1.0-_T,2.0) + k[9];

	P = exp(1.0/_T*x1/x2 - x3/x4);
...
出力:部分領域2と3の境界の値
**************************/
double PropertyWater::L_func(double _T)
{
	double P;
	P = L0 + L1 * _T + L2 * _T * _T;
	return P;
}
double PropertyWater::Ld_func(double _T )
{
	double P;
	P = L1 + 2.0 * L2 * _T;
	return P;
...
出力:換算比容積[-]
****************************/
double PropertyWater::chi1( double _P , double _T )
{
	double Y,Z,x1,x2,x3,x4,x5,v;

	Y = 1.0 - a[1] * pow(_T,2.0)-a[2]*pow(_T,-6.0);
	Z = Y + sqrt(a[3]*pow(Y,2.0)-2.0*a[4]*_T+2.0*a[5]*_P);

	x1=A[11]*a[5]*pow(Z,-5.0/17.0);
	x2=A[12]+A[13]*_T+A[14]*pow(_T,2.0)+A[15]*pow(a[6]-_T,10.0)+A[16]*pow(a[7]+pow(_T,19.0),-1.0);
	x3=-pow(a[8]+pow(_T,11.0),-1.0)*(A[17]+2.0*A[18]*_P+3.0*A[19]*_P*_P);
	x4=-A[20]*pow(_T,18.0)*(a[9]+pow(_T,2.0))*(-3.0*pow(a[10]+_P,-4.0)+a[11]);
	x5=3.0*A[21]*(a[12]-_T)*_P*_P+4.0*A[22]*pow(_T,-20.0)*pow(_P,3.0);
...
出力:換算エントロピ[-]
****************************/
double PropertyWater::sigma1(double _P,double _T){
	int nyu;
	double Y,Z,Yd,x1,x2,x3,x4,x5,x6,x7,x8,x9,s;

	Y=1.0-a[1]*pow(_T,2.0)-a[2]*pow(_T,-6.0);
	Z=Y+sqrt(a[3]*pow(Y,2.0)-2.0*a[4]*_T+2.0*a[5]*_P);
	Yd=-2.0*a[1]*_T+6.0*a[2]*pow(_T,-7.0);
	...
	{
		x3+=(nyu-1.0)*A[nyu]*pow(_T,nyu-2.0);
	}
	x4=A[11]*((5.0/12.0*Z-(a[3]-1.0)*Y)*Yd+a[4])*pow(Z,-5.0/17.0);
	x5=(-A[13]-2.0*A[14]*_T+10.0*A[15]*pow(a[6]-_T,9.0)+19.0*A[16]*pow(a[7]+pow(_T,19.0),-2.0)*pow(_T,18.0))*_P;
	x6=11.0*pow(a[8]+pow(_T,11.0),-2.0)*pow(_T,10.0)*(A[17]*_P+A[18]*_P*_P+A[19]*_P*_P*_P);
	x7=A[20]*pow(_T,17.0)*(18.0*a[9]+20.0*_T*_T)*(pow(a[10]+_P,-3.0)+a[11]*_P);
	x8=A[21]*_P*_P*_P;
	x9=20.0*A[22]*pow(_T,-21.0)*pow(_P,4.0);
...
出力:換算エンタルピ[-]
****************************/
double PropertyWater::epsilon1(double p,double t)
{
	int nyu;
...
出力:換算比容積[-]
****************************/
double PropertyWater::chi2(double p,double t)
{
	int myuu,nyu,lambda;
...
出力:換算エントロピ[-]
****************************/
double PropertyWater::sigma2(double p,double t)
{
	int myuu,nyu,lambda;
...
出力:換算エンタルピ[-]
****************************/
double PropertyWater::epsilon2(double p,double t)
{
	int myuu,nyu,lambda;
...
出力:換算定圧比熱[-]
****************************/
double PropertyWater::phi1(double p,double t)
{
	int nyu;
...
出力:換算定圧比熱[-]
****************************/
double PropertyWater::phi2(double p,double t)
{
	int nyu;
...
出力:粘性係数[μPas]
*******************************/
double PropertyWater::troumyuu(double t,double rou)
{
	int i,j,k;
...
出力:熱伝達率[W/mK]
*******************************/
double PropertyWater::troulambda(double t,double rou)
{
...
*********************************/
//飽和圧力[kPa]
double PropertyWater::t_p(double _T)
{
	double P;
	_T=(_T+T0)/Tc;
	P=k_func(_T)*Pc;
...
//飽和温度[℃]
double PropertyWater::p_t(double _P)
{
	double move_temp;
	int i;double x[2],y[2],dec=1.0+1e-7,eps=1e-9;
	double T=pow(_P,0.2)*14.13-44.0;
	do{
		for(i=0;i<=1;i++)
		{
			x[i] = T*((i==0)?(dec):(1.0));
		...
		if( 100.0 < y[0]*(x[1]-x[0])/(y[1]-y[0]) ){
			move_temp = 100.0;
		}else if( y[0]*(x[1]-x[0])/(y[1]-y[0]) < -100.0 ){
			move_temp = -100.0;
		}else{
			move_temp = y[0]*(x[1]-x[0])/(y[1]-y[0]);
		}
...
*********************************/
//圧縮液
double PropertyWater::sc_vl(double _P,double _T){
	_P*=1000;
	double v;
	_P=_P/Pc;
	_T=(_T+T0)/Tc;
...
//過熱蒸気
double PropertyWater::sh_vv(double _P , double _T){
	_P*=1000;
	double v;
	_P=_P/Pc;
	_T=(_T+T0)/Tc;
...
//飽和液
 double PropertyWater::sat_vl(double t)
{
	double p,v;
	p=t_p(t);
	v=sc_vl(p,t);
...
//飽和蒸気
 double PropertyWater::sat_vv(double t)
{
	double p,v;
	p=t_p(t);
	v=sh_vv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_roul(double p,double t)
{
	return 1.0/sc_vl(p,t);
}
//過熱蒸気
 double PropertyWater::sh_rouv(double p,double t)
{
	return 1.0/sh_vv(p,t);
}
//飽和液
 double PropertyWater::sat_roul(double t)
{
	return 1.0/sat_vl(t);
}
//飽和蒸気
 double PropertyWater::sat_rouv(double t)
{
	return 1.0/sat_vv(t);
}

 double PropertyWater::rou_Ph( double P , double h ){
	...
	if( h <= hl ){//圧縮液の場合
		rou = this->sc_roul( P , Tsat );
	}else if( hl < h && h <= hv ){//二相域の場合
		x = ( h - hl ) / ( hv - hl );
		roul = this->sat_roul( Tsat );
		rouv = this->sat_rouv( Tsat );
		rou = ( roul * rouv ) / ( rouv + ( roul - rouv ) * x );
	}else if( hv < h ){//過熱の場合
		rou = this->sh_rouv( P , Tsat );
	}
...
*********************************/
//圧縮液
 double PropertyWater::sc_sl(double p,double t)
{
	p*=1000;
	double s;
	p=p/Pc;
...
//過熱蒸気
 double PropertyWater::sh_sv(double p,double t)
{
	p*=1000;
	double s;
	p=p/Pc;
...
//飽和液
 double PropertyWater::sat_sl(double t)
{
	double p,s;
	p=t_p(t);
	s=sc_sl(p,t);
...
//飽和蒸気
 double PropertyWater::sat_sv(double t)
{
	double p,s;
	p=t_p(t);
	s=sh_sv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_hl(double p,double t)
{
	p*=1000;
	double h;
	p=p/Pc;
...
//過熱蒸気
 double PropertyWater::sh_hv(double p,double t)
{
	p*=1000;
	double h;
	p=p/Pc;
...
//飽和液
 double PropertyWater::sat_hl(double t)
{
	double p,h;
	p=t_p(t);
	h=sc_hl(p,t);
...
//飽和蒸気
 double PropertyWater::sat_hv(double t)
{
	double p,h;
	p=t_p(t);
	h=sh_hv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_cpl(double p,double t)
{
	p*=1000;
	double cp;
	p=p/Pc;
...
//過熱蒸気
 double PropertyWater::sh_cpv(double p,double t)
{
	p*=1000;
	double cp;
	p=p/Pc;
...
//飽和液
 double PropertyWater::sat_cpl(double t)
{
	double p,cp;
	p=t_p(t);
	cp=sc_cpl(p,t);
...
//飽和蒸気
 double PropertyWater::sat_cpv(double t)
{
	double p,cp;
	p=t_p(t);
	cp=sh_cpv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_myul(double p,double t)
{
	double myuu,rou;
	rou=sc_roul(p,t);
	t=t+T0;
...
//過熱蒸気
 double PropertyWater::sh_myuv(double p,double t)
{
	double myuu,rou;
	rou=sh_rouv(p,t);
	t=t+T0;
...
//飽和液
 double PropertyWater::sat_myul(double t)
{
	double p,myuu;
	p=t_p(t);
	myuu=sc_myul(p,t);
...
//飽和蒸気
 double PropertyWater::sat_myuv(double t)
{
	double p,myuu;
	p=t_p(t);
	myuu=sh_myuv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_laml(double p,double t)
{
	double lambda,rou;
	rou=sc_roul(p,t);
	t=t+T0;
...
//過熱蒸気
 double PropertyWater::sh_lamv(double p,double t)
{
	double lambda,rou;
	rou=sh_rouv(p,t);
	t=t+T0;
...
//飽和液
 double PropertyWater::sat_laml(double t)
{
	double p,lambda;
	p=t_p(t);
	lambda=sc_laml(p,t);
...
//飽和蒸気
 double PropertyWater::sat_lamv(double t)
{
	double p,lambda;
	p=t_p(t);
	lambda=sh_lamv(p,t);
...
*********************************/
//圧縮液
 double PropertyWater::sc_nyul(double p,double t)
{
	double nyu;
...
//過熱蒸気
 double PropertyWater::sh_nyuv(double p,double t)
{
	double nyu;
...
//飽和液
 double PropertyWater::sat_nyul(double t)
{
	double nyu;
...
//飽和蒸気
 double PropertyWater::sat_nyuv(double t)
{
	double nyu;
...
*********************************/
//圧縮液
 double PropertyWater::sc_Prl(double p,double t)
{
	double pr;
...
//過熱蒸気
 double PropertyWater::sh_Prv(double p,double t)
{
	double pr;
...
//飽和液
 double PropertyWater::sat_Prl(double t)
{
	double pr;
...
//飽和蒸気
 double PropertyWater::sat_Prv(double t)
{
	double pr;
...
//!!基準は常に圧縮液であることに注意!!
//圧縮液
 double PropertyWater::sc_el(double p,double t,double p0,double t0)
{
	p*=1000;
	double e;
...
//過熱蒸気
 double PropertyWater::sh_ev(double p,double t,double p0,double t0)
{
	p*=1000;
	double e;
...
//飽和液
 double PropertyWater::sat_el(double t,double p0,double t0)
{
	double e;
...
//飽和蒸気
 double PropertyWater::sat_ev(double t,double p0,double t0)
{
	double e;
...
*********************************/
//圧縮液
 double PropertyWater::sc_tl(double p,double h)
{
	int i;double x[2],y[2],dec=1.0+1e-7,eps=1.0e-8;
...
//加熱蒸気
 double PropertyWater::sh_tv(double p,double h)
{
	int i;double x[2],y[2],dec=1.0+1e-7,eps=1.0e-9;
	double t=pow(p,0.2)*14.13-44.0;
	do{
		for(i=0;i<=1;i++)
		{
...
double PropertyWater::T_Ph( double P , double h ){
